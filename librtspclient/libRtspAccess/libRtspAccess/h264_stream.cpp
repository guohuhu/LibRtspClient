...
//TODO
int more_rbsp_data(h264_stream_t* h, bs_t* b) { return 0; };
unsigned int next_bits(bs_t* b, int n) { return 0; };



//7.3.1 NAL unit syntax
void 
read_nal_unit(h264_stream_t* h, unsigned char* buf, int size)
{
//	unsigned char* rbsp_buf = NULL;
	int rbsp_size = 0;
	int i;
	nal_t* nal = &h->nal;
...
// 	bs_init(b, buf, size);
	...
	unsigned int temp = bs_read_f(&b,1);

	nal->forbidden_zero_bit = temp;
	nal->nal_ref_idc = bs_read_u(&b,2);
	nal->nal_unit_type = bs_read_u(&b,5);

	unsigned char rbsp_buf[524288] = {0};

	//rbsp_buf = (unsigned char *)malloc(size);
	...
	for( i = 1; i < size; i++ )
	{
		...
		{ // && next_bits( 24 ) == 0x000003
			// if (buf[i+2] != 0x03) { /* error */ }; // nal->emulation_prevention_three_byte = bs_read_f(8); /* equal to 0x03 */
			rbsp_buf[ rbsp_size++ ] = buf[i];
			rbsp_buf[ rbsp_size++ ] = buf[i+1];
			i += 2;
		} 
		else
		{
...
void 
read_seq_parameter_set_rbsp(h264_stream_t* h, bs_t* b)
{
	sps_t* sps = &h->sps;
...
void
read_vui_parameters(h264_stream_t* h, bs_t* b)
{
	sps_t* sps = &h->sps;
...
void
read_hrd_parameters(h264_stream_t* h, bs_t* b)
{
	sps_t* sps = &h->sps;
	int SchedSelIdx;
...
void 
read_pic_parameter_set_rbsp(h264_stream_t* h, bs_t* b) {
	pps_t* pps = &h->pps;
	...
	if( pps->num_slice_groups_minus1 > 0 ) {
		...
		if( pps->slice_group_map_type == 0 )
			...
			for( i = 0; i <= pps->pic_size_in_map_units_minus1; i++ )
				pps->slice_group_id[ i ] = bs_read_u(b, (int)ceil( log2((pps->num_slice_groups_minus1 + 1) ) ) ); // was u(v)
		}
	}
...
void 
read_access_unit_delimiter_rbsp(h264_stream_t* h, bs_t* b) {
...
void 
read_end_of_seq_rbsp(h264_stream_t* h, bs_t* b) {
}

//7.3.2.6 End of stream RBSP syntax
void 
read_end_of_stream_rbsp(h264_stream_t* h, bs_t* b) {
}

//7.3.2.7 Filler data RBSP syntax
void 
read_filler_data_rbsp(h264_stream_t* h, bs_t* b) {
	int ff_byte; //FIXME
...
void 
read_slice_layer_rbsp(h264_stream_t* h, bs_t* b) {
...
void 
read_rbsp_slice_trailing_bits(h264_stream_t* h, bs_t* b) {
	read_rbsp_trailing_bits(h, b);
	int cabac_zero_word;
...
void 
read_rbsp_trailing_bits(h264_stream_t* h, bs_t* b) {
...
void 
read_slice_header(h264_stream_t* h, bs_t* b) {
	...
	if( pps->num_slice_groups_minus1 > 0 &&
		...
		sh->slice_group_change_cycle = 
			bs_read_u(b, (int)ceil( log2((pps->pic_size_in_map_units_minus1 + pps->slice_group_change_rate_minus1 + 1 )) ) ); // was u(v) // FIXME add 2?
}
...
//7.3.1 NAL unit syntax
void 
write_nal_unit(h264_stream_t* h, unsigned char* buf, int size) {
	nal_t* nal = &h->nal;
	...
	int i;
	for( i = 1; i < size; i++ ) {
		if( i + 2 < size && buf[i] == 0x03 && buf[i+1] == 0x00 && buf[i+2] == 0x00 ) { // && next_bits( 24 ) == 0x000003
			if (buf[i+2] != 0x03) { error }; // nal->emulation_prevention_three_byte = bs_write_f(8);
			rbsp_buf[ rbsp_size++ ] = buf[i];
			rbsp_buf[ rbsp_size++ ] = buf[i+1];
			i += 2;
		} else
...
//7.3.2.1 Sequence parameter set RBSP syntax
void 
write_seq_parameter_set_rbsp(h264_stream_t* h, bs_t* b)
{
	sps_t* sps = &h->sps;
...
//7.3.2.2 Picture parameter set RBSP syntax
void 
write_pic_parameter_set_rbsp(h264_stream_t* h, bs_t* b) {
	pps_t* pps = &h->pps;
	...
	if( pps->num_slice_groups_minus1 > 0 ) {
		...
		if( pps->slice_group_map_type == 0 )
			...
			for( i = 0; i <= pps->pic_size_in_map_units_minus1; i++ )
				bs_write_u(b, (int)ceil( log2(pps->num_slice_groups_minus1 + 1 ) ), pps->slice_group_id[ i ] ); // was u(v)
		}
	}
...
//7.3.2.4 Access unit delimiter RBSP syntax
void 
write_access_unit_delimiter_rbsp(h264_stream_t* h, bs_t* b) {
	int primary_pic_type = 0; //FIXME
...
//7.3.2.5 End of sequence RBSP syntax
void 
write_end_of_seq_rbsp(h264_stream_t* h, bs_t* b) {
}

//7.3.2.6 End of stream RBSP syntax
void 
write_end_of_stream_rbsp(h264_stream_t* h, bs_t* b) {
}

//7.3.2.7 Filler data RBSP syntax
void 
write_filler_data_rbsp(h264_stream_t* h, bs_t* b) {
	int ff_byte = 0; //FIXME
	while( next_bits(b, 8) == 0xFF )
...
//7.3.2.8 Slice layer without partitioning RBSP syntax
void 
write_slice_layer_rbsp(h264_stream_t* h, bs_t* b) {
...
//7.3.2.10 RBSP slice trailing bits syntax
void 
write_rbsp_slice_trailing_bits(h264_stream_t* h, bs_t* b) {
	write_rbsp_trailing_bits(h, b);
...
//7.3.2.11 RBSP trailing bits syntax
void 
write_rbsp_trailing_bits(h264_stream_t* h, bs_t* b) {
...
//7.3.3 Slice header syntax
void 
write_slice_header(h264_stream_t* h, bs_t* b) {
	slice_header_t* sh = &h->sh;
	sps_t* sps = &h->sps;
	pps_t* pps = &h->pps;
	nal_t* nal = &h->nal;
	...
	if( pps->num_slice_groups_minus1 > 0 &&
		pps->slice_group_map_type >= 3 && pps->slice_group_map_type <= 5) {
		bs_write_u(b, (int)ceil( log2( pps->pic_size_in_map_units_minus1 +  
...
void debug_bytes(unsigned char* buf, int len)
{
	int i;
	for (i = 0; i < len; i++)
	{
